...
**当前状态**：`ABD-024` 至 `ABD-032`、`ABD-035` 至 `ABD-045` 已完成 daemon、provider deployment boundary、全部 P1 typed server-read/action source 及 runtime/inbound、grant-bound message、action recovery、provider isolation、cancellation、privacy e2e gate，以及 P2 的 `message.send_text`、`message.send_quote`、`message.send_at` 和 `conversation.mark_read`。OpenIM 未公开 server unread count，故 `conversation.unread` 继续 `not_validated`。这些 action 只在 manifest、显式 grant 和对应 handler 共同允许时公开；引用额外要求会话和原消息 target，已读额外要求有限消息窗口。默认入站 policy 仍只授予 `message.history`。下一交付是 `ABD-046` 的附件授权基础设施；P3 的消息媒体、会话、社交和群组写入会在各自固定 SDK/server action 已确认后，按一个 typed method 一个 task 的规则进入本清单。

## 执行顺序

`ABD-001` 完成后，Foundation 中标记 `[P]` 的 task 可并行；P0 checkpoint 后，事件账本和 grant/proxy 可按依赖并行，US-01 是首个可交付闭环。US-02 先交付 capability 执行面和首个 action handler，再由共享 typed read service 同时补齐 US-02 与 US-03；`ABD-024` 至 `ABD-032`、`ABD-035` 至 `ABD-045` 已完成 daemon、provider MCP、部署边界、全部 P1 server-read/action source 及 lifecycle/inbound/grant/action/isolation/cancellation/privacy e2e。P2 的消息发送、引用、群 @ 和已读边界已完成，接下来由 `ABD-046` 交付附件边界；P3 只在相应固定 action source 已确认后，再按 IM 领域和一个 typed method 一个 task 的规则推进。
