...
| 完成 | ID | 状态 | 场景 | 任务与记录 | 依赖 | 完成条件 |
| --- | --- | --- | --- | --- | --- | --- |
| [ ] | ABD-046 | ready | US-02 | 建立受 grant 限制的消息附件 metadata、quota 与不透明引用基础设施。详见 [`issues/ABD-046-attachment-foundation.md`](issues/ABD-046-attachment-foundation.md)。 | ABD-041 | provider 不能传递任意本地路径；每个附件引用绑定 profile、run、额度和有效期；正文与文件内容不进入 control DB、审计或日志。 |
...
**当前状态**：`ABD-024` 至 `ABD-032`、`ABD-035` 至 `ABD-045` 已完成 daemon、provider deployment boundary、全部 P1 typed server-read/action source 及 runtime/inbound、grant-bound message、action recovery、provider isolation、cancellation、privacy e2e gate，以及 P2 的 `message.send_text`、`message.send_quote`、`message.send_at` 和 `conversation.mark_read`。OpenIM 未公开 server unread count，故 `conversation.unread` 继续 `not_validated`。这些 action 只在 manifest、显式 grant 和对应 handler 共同允许时公开；引用额外要求会话和原消息 target，已读额外要求有限消息窗口。默认入站 policy 仍只授予 `message.history`。P2 仍缺附件基础设施和消息控制能力；P3 的媒体/文件、会话设置、好友/黑名单和群组写入已按能力族建立可追溯 issue。

## 执行顺序

`ABD-001` 完成后，Foundation 中标记 `[P]` 的 task 可并行；P0 checkpoint 后，事件账本和 grant/proxy 可按依赖并行，US-01 是首个可交付闭环。US-02 先交付 capability 执行面和首个 action handler，再由共享 typed read service 同时补齐 US-02 与 US-03；`ABD-024` 至 `ABD-032`、`ABD-035` 至 `ABD-045` 已完成 daemon、provider MCP、部署边界、全部 P1 server-read/action source 及 lifecycle/inbound/grant/action/isolation/cancellation/privacy e2e。`ABD-047` 不依赖附件基础设施；`ABD-048` 在 `ABD-046` 完成后推进。P3 的 `ABD-049` 至 `ABD-052` 位于不同 IM 领域，可在 `ABD-041` 后并行；`ABD-053` 与群成员关系共享实现目录，在 `ABD-052` 的固定 action source 已确认后推进。
