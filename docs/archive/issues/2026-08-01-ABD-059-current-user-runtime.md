...
- 每个 run 继续生成新的私有 `CODEX_HOME`、固定 MCP 配置和单连接 bridge；复制当前用户 Codex 登录材料及非 MCP 模型/供应商配置，不继承其 MCP 与 history 配置。
...
- provider adapter 在每个 run 复制 `auth.json`，继承非 MCP 模型/供应商配置，生成私有工作目录和固定 MCP 配置；源 Codex MCP 与 history 配置不继承。
