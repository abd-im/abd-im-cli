...
Prefer one simple, supported path for each product capability. User-facing setup and lifecycle commands must hide stable integration plumbing behind safe defaults, and must not introduce secondary identities or onboarding concepts that the requested product behavior does not require. Do not retain alternative modes or operational variants unless a demonstrated requirement justifies their user, maintenance, and verification cost.
